...
# KGSM-Container management file for Empyrion: Galactic Survival (empyrion)
#
...
export instance_launch_dir="$instance_install_dir/DedicatedServer"

export instance_steam_app_id=530870
...
export instance_executable_file="$instance_launch_dir/EmpyrionDedicated.exe"
export instance_executable_arguments="-batchmode -nographics -dedicated $instance_saves_dir/dedicated.yaml -logFile $instance_logs_redirect"
...
function _start() {
  ...
  # Check for dedicated.yaml and copy default if needed
  if [ ! -f "${instance_saves_dir}/dedicated.yaml" ]; then
    echo "${instance_saves_dir}/dedicated.yaml not found. Copying default file."
    cp "${instance_install_dir}/dedicated.yaml" "${instance_saves_dir}/dedicated.yaml" 2>&1
  fi
  ...
  # Start log reading in a completely detached process that continues after exec
  (
    # First fork
    (
      # Second fork - completely detaches from parent
      exec 0</dev/null # Only redirect stdin to /dev/null

      # Create a new session to detach from terminal control signals
      setsid bash -c "/opt/empyrion/empyrion.manage.sh --logs --follow" 2>&1 &
    ) &
  )

  # Give the log process a moment to start
  sleep 1
