...
# KGSM-Container management file for V Rising (vrising)
#
...
export instance_executable_file="$instance_launch_dir/VRisingServer.exe"
export instance_executable_arguments="-persistentDataPath $instance_saves_dir"
...
function _start() {
  ...
  if [[ ! -d "${instance_saves_dir}/Settings" ]]; then
    echo "Creating saves directory at ${instance_saves_dir}"
    mkdir -p "${instance_saves_dir}/Settings"
  fi

  # Check for ServerGameSettings.json and copy default if needed
  if [ ! -f "${instance_saves_dir}/Settings/ServerGameSettings.json" ]; then
    echo "${instance_saves_dir}/Settings/ServerGameSettings.json not found. Copying default file."
    cp "${instance_install_dir}/VRisingServer_Data/StreamingAssets/Settings/ServerGameSettings.json" "${instance_saves_dir}/Settings/" 2>&1
  fi

  # Check for ServerHostSettings.json and copy default if needed
  if [ ! -f "${instance_saves_dir}/Settings/ServerHostSettings.json" ]; then
    echo "${instance_saves_dir}/Settings/ServerHostSettings.json not found. Copying default file."
    cp "${instance_install_dir}/VRisingServer_Data/StreamingAssets/Settings/ServerHostSettings.json" "${instance_saves_dir}/Settings/" 2>&1
  fi
