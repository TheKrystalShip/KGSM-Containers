...
export instance_executable_arguments="$instance_launch_dir/AbioticFactorServer-Win64-Shipping.exe -tcp -useperfthreads -MaxServerPlayers=$max_server_players -PORT=$port -QueryPort=$query_port -ServerPassword=$server_password -SteamServerName=$steam_server_name -WorldSaveName=$world_save_name $additional_args"
